...
'''
    ...
    detected faces are asked to be tagged and saved
        ...
        +reduced amount of classifiers                                                                                                  - done
        +hot to encode classes? - what are unclassified faces? neg-examples?                                                            - done

        imshow could receive a resized image if x < 100: upscale elif x > 800: downscale                                                - done
...
def tag_and_save(roi_gray):
    ...
    if roi_gray[1] < 100:
        factor = 100/roi_gray.shape[1]
    elif roi_gray[1] > 800:
        factor = 800/roi_gray.shape[1]
    shw = cv2.resize(roi_gray,None,fx=factor, fy=factor, interpolation = cv2.INTER_AREA)

    cv2.imshow('roi_gray',shw)
